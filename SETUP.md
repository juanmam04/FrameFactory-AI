...
1. En `.env`: `COMFYUI_URL=http://127.0.0.1:8188` (ya lo tenés).
2. **Prender ComfyUI** (en otra terminal, dejalo abierto mientras usás FrameFactory):

   **Si ya tenés ComfyUI clonado:**
   ```bash
   cd /ruta/donde/está/ComfyUI
   python main.py --port 8188
   ```
   Si no pasás `--port`, por defecto usa 8188.
   ...
   **Si todavía no lo tenés instalado:**
   ...
   cd ComfyUI
   pip install -r requirements.txt
   # Descargá al menos un checkpoint (ej. SD 1.5) en ComfyUI/models/checkpoints/
   python main.py --port 8188
   ```
   Cuando veas algo como `To see the GUI go to: http://127.0.0.1:8188`, ComfyUI está listo. Dejá esa terminal abierta y en la app marcá que no quieras «Saltar generación de imágenes» para que se generen imágenes reales.
